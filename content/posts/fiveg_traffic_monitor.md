...
不想拉宽带了，自己的手机套餐一个月500G的流量，也用不完。于是收了一个联通CPE：VN007+ 。插上自己的副卡，感觉用起来很不错，这个CPE也具有短信收发功能。但是最让人焦虑的就是「到底用了多少」「会不会悄悄超额」。虽然路由器后台能查，但每次都要开浏览器、登录、点菜单，实在太麻烦。
...
## 项目地址
项目地址：<https://gitee.com/assad2008/5g-traffic-monitoring-system>
...
写这个工具的初衷很朴素--不想老登录CPE的后台去看流量用了多少。但顺手把短信和 Web 接口都做上之后，这台 CPE 才真正像个「能用的设备」：验证码不用再翻后台，本机脚本也能通过 HTTP API 自动发短信。
